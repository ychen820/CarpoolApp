...
- (void)viewDidLoad {
    ...
    self.navigationItem.title=self.recipient.firstName;
...
-(void)loadAvatars{
    ...
    [[[[[FireBaseManager sharedFireBaseManager].databaseRef root]child:@"users"]child:self.senderId]observeSingleEventOfType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
        NSString *imgURLStr=snapshot.value[@"photoURL"];
        if(imgURLStr!=nil){
          ...
          [[[NSURLSession sharedSession]dataTaskWithURL:imgURL completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
              if(error){
                  NSLog(@"Error loading avatar:%@",error.description);
              }
              else{
              UIImage *image=[UIImage imageWithData:data];
              self.senderAvatar=[JSQMessagesAvatarImageFactory avatarImageWithImage:image diameter:40];
                  [[[[[FireBaseManager sharedFireBaseManager].databaseRef root]child:@"users"]child:self.recipient.uid]observeSingleEventOfType:FIRDataEventTypeValue withBlock:^(FIRDataSnapshot * _Nonnull snapshot) {
